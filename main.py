        ...
        return [
            {
                ...
                "dimensions_points": {
                    ...
                    "x0": round(page.rect.x0, 2),
                    "y0": round(page.rect.y0, 2),
                    "x1": round(page.rect.x1, 2),
                    "y1": round(page.rect.y1, 2)
                },
                "dimensions_pixels": {
                    "width": int((page.rect.width * 150) / 72),
                    "height": int((page.rect.height * 150) / 72),
                    "dpi": 150
                },
                "dimensions_mm": {
                    "width": round(page.rect.width * 0.352778, 2),
                    "height": round(page.rect.height * 0.352778, 2)
                }
            }
...
def point_to_dict(p, precision: int = 2) -> dict:
    ...
    try:
        return {
            "x": round(float(p.x if hasattr(p, 'x') else p[0]), precision),
            "y": round(float(p.y if hasattr(p, 'y') else p[1]), precision)
        }
...
def rect_to_dict(r, precision: int = 2) -> dict:
    ...
    try:
        x0, y0, x1, y1 = (r.x0, r.y0, r.x1, r.y1) if hasattr(r, 'x0') else r[:4]
        return {
            "x0": round(float(x0), precision),
            "y0": round(float(y0), precision),
            "x1": round(float(x1), precision),
            "y1": round(float(y1), precision),
            "width": round(float(x1) - float(x0), precision),
            "height": round(float(y1) - float(y0), precision)
        }
    except (ValueError, TypeError, AttributeError, IndexError):
        return {"x0": 0.0, "y0": 0.0, "x1": 0.0, "y1": 0.0, "width": 0.0, "height": 0.0}
...
def minify_output(data: Dict, minify: bool = True, remove_non_essential: bool = True) -> str:
    ...
    if remove_non_essential:
        ...
        if "summary" in output_data:
            ...
            if "coordinate_bounds" not in output_data["summary"]:
                output_data["summary"].update(calculate_coordinate_bounds(output_data))
...
async def extract_all(
    ...
    try:
        ...
        # Gebruik MyPdfModel voor dimensies
        ...
        # Toegevoegd: Bereken coordinate_bounds
...
async def root():
    return {
        ...
        "version": "1.0.0",
        "description": "Extracts text and vector data from technical drawings with minification and coordinate info",
        "endpoints": {
            "/": "This page",
